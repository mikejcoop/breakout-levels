...
function createBall({ upwards = true, position = null, angle = null } = {}) {
...
function drawPowerUps() {
  ...
  powerUps.forEach((powerUp) => {
    ...
    ctx.lineWidth = 2;
    roundedRect(ctx, powerUp.x, powerUp.y, POWER_UP_SIZE, POWER_UP_SIZE, 6);
    ctx.stroke();
    ...
    for (let i = 0; i < bricks.length; i += 1) {
      ...
      if (
        ...
        if (!piercingActive) {
          ...
          const overlapRight =
            brick.x + brickSettings.width - (ball.x - ball.radius);
          const overlapTop = ball.y + ball.radius - brick.y;
          const overlapBottom =
            brick.y + brickSettings.height - (ball.y - ball.radius);
...
function renderLeaderboard(entries) {
  ...
  if (!entries || entries.length === 0) {
    showLeaderboardStatus(
      "No runs recorded yet. Be the first to claim the top spot!"
    );
    return;
  }
...
async function importInstantDB() {
  const sources = [
    ...
    "https://cdn.skypack.dev/@instantdb/core@latest",
  ];
...
async function submitLeaderboardEntry({ victory }) {
  ...
  const entryId =
    typeof client.id === "function" ? client.id() : generateLocalId();
  const createdAt =
    typeof client.now === "function" ? client.now() : new Date().toISOString();
  ...
  if (instantDbTx && instantDbTx.leaderboard) {
    try {
      ...
      console.warn(
        "InstantDB tx helper failed, using fallback mutation",
        txError
      );
    }
  }
